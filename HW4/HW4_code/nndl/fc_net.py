...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        ...
        # Affine Layer
        out, cache[f'affine{i}'] = affine_forward(out, W, b)

        # Batch Normalization (if enabled)
        if self.use_batchnorm:
            gamma, beta = self.params[f'gamma{i}'], self.params[f'beta{i}']
            out, cache[f'batchnorm{i}'] = batchnorm_forward(out, gamma, beta, self.bn_params[i-1])

        # ReLU Activation
        out, cache[f'relu{i}'] = relu_forward(out)

        # Dropout (if enabled)
        if self.use_dropout:
            out, dropout_cache[f'dropout{i}'] = dropout_forward(out, self.dropout_param)

    # Last layer (affine -> softmax)
    scores, cache[f'affine{self.num_layers}'] = affine_forward(out, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
